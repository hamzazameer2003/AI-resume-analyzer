  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate">Guided roles based on your resume analysis.</p>
      </div>
      {error && <p className="text-sm text-red-500">{error}</p>}
      <div className="grid gap-4 md:grid-cols-2">
        {suggestions.map((item) => (
          <div key={item} className="rounded-2xl border border-white/70 bg-white/80 p-6 shadow-sm">
            {item}
          </div>
        ))}
        {!suggestions.length && (
